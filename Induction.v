...
(* Induction only works on inductive definitions, destruct works on
   others. Also, induction gives an induction hypothesis, destruct does not. *)
...
Proof.
  ...
  Case "m = 0".
    ...
    reflexivity.
  Case "m = S m'".
    simpl. rewrite IHm'. rewrite plus_assoc. rewrite plus_assoc.
    assert (n + m' = m' + n) as H.
      SCase "n and m' commute under +".
      rewrite plus_comm. reflexivity.
...
Proof.
  ...
  assert (forall x: nat, evenb (S x) = negb (evenb x)) as H1.
