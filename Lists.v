...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  match l1 with
    | nil => l2
    | h1 :: rem1 => match l2 with
                      | nil => l1
                      | h2 :: rem2 => h1 :: h2 :: (alternate rem1 rem2)
                    end
  end.
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
    | nil => s
    | h :: rst => match (beq_nat h v) with
                    | true => rst
                    | false => h :: (remove_one v rst)
                  end
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
    | nil => s
    | h :: rst => match (beq_nat h v) with
                    | true => remove_all v rst
                    | false => h :: (remove_all v rst)
                  end
  end. 
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
    | nil => true
    | h :: rst => match (ble_nat (count h s1) (count h s2)) with
                    | true => subset rst s2
                    | false => false
                  end
  end.
